...
export const BlockchainProvider = ({ children }) => {
    ...
    eggNFT?.balanceOf().then((res) => {
      ...
      for (let i = 0; i < Number(res); i++) {
        eggNFT?.tokenOfOwnerByIndex(i).then((res) => {
          _EGGS.push({
            image: `https://gateway.pinata.cloud/ipfs/Qmdvr95JsHCnuKVEdAX784qcYDK9HndbpPMT7FDopdF8eQ/eggo${zeros.slice(
              String(res).length
            )}${res}.png`,
          });
        });
  ...
  }, [eggNFT, chainId, account]); // Include eggNFT in the dependency array
